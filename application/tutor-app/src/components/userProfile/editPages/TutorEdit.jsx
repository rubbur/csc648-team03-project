...
const TutorEdit = ({ isTutor }) => {
  ...
  useEffect(() => {
    const getTutorData = async () => {
      ...
      setCourseNumbers([
        ...result.data.userData[0].courses
          .split(" ")
  ...
  const handleEditTutor = async () => {
    ...
    const result = await axios.post(
      `${process.env.REACT_APP_BACKEND_URL}/user/editTutorAbilities`,
      {
        id: cookie.get("userId"),
  ...
  const handleBecomeTutor = async () => {
    const result = await axios.post(
      `${process.env.REACT_APP_BACKEND_URL}/user/becomeTutor`,
  ...
  return (
    <div className="tutor-edit edit-form">
              ...
              courseNumbers.map((course, index) => {
                if (course !== "")
                  return (
                    <CourseCard
                      courseNumber={course}
                      deleteCourseNumber={deleteCourseNumber}
                      key={index}
                      index={index}
                    />
                  );
      ...
      <button
        onClick={isTutor ? handleEditTutor : handleBecomeTutor}
        className="submit-tutor-button"
      >
        {isTutor ? "Apply Changes" : "Register as Tutor"}
      </button>
    </div>
  );
};
  ...
  return (
    <div className="course-card">
      ...
      <button
        ...
        onClick={() => deleteCourseNumber(index)}
      >
        X
      </button>
    </div>
