...
const getTutorReviews = async (req, res) => {
  ...
  try {
    let q =
      "SELECT * FROM tutor_reviews WHERE tutor_id = ? ORDER BY time_stamp DESC";
    const reviews = await db.query(q, [id]);
    const results = reviews[0].map((review) => {
      let formattedDate = review.time_stamp
        .toISOString()
        .slice(0, 10)
        .split("-");
      review = {
    ...
    console.log(
      "error retrieving reviews for tutor with id: " + id + " error: " + err,
    );
  }
};
...
const getPostById = async (req, res) => {
  ...
  const q =
    "SELECT tutor_posts.*, users.img_url, users.username FROM tutor_posts JOIN users ON tutor_posts.tutor_id = users.id WHERE tutor_posts.post_id = ?";
  try {
    ...
    console.log(
      "Error retrieving posts for tutor with id: " + postId + " error: " + err,
    );
...
const getPostByTutorId = async (req, res) => {
  ...
  const q =
    "SELECT subject, post_id, hourly_rate FROM tutor_posts WHERE tutor_posts.tutor_id = ?";
  try {
    const posts = await db.query(q, [tutorId]);
    res.send({ success: true, postData: posts[0] });
  } catch (err) {
    console.log(
      "error retrieving posts for tutor with id: " + tutorId + " error: " + err,
    );
  }
};
...
const uploadFile = async (req, res) => {
  ...
  if (file.mimetype === "application/pdf" && updateColumn === "flier_url") {
    ...
    file.mimetype.substring(0, 5) === "video" &&
  ...
  file.mv(
    `../tutor-app/public/${destinationFolder}/${newFileName}`,
    async (err) => {
      if (err) {
        console.error(err);
